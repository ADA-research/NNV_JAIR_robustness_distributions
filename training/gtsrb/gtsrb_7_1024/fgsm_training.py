...
from ....networks_pytorch.gtsrb_7_1024 import GTSRB_7_1024
...
def objective(trial):
    ...
    test_module = StandardTestModule(attack=FGSMAttack(), epsilon=attack_epsilon)
...
if __name__ == "__main__":
    ...
    training_parameters = AttributeDict(learning_rate=0.00066, 
                                        weight_decay=0.000067, 
                                        scheduler_step_size=10, 
                                        scheduler_gamma=0.467,
                                        attack_epsilon=ATTACK_EPSILON,
                                        batch_size=BATCH_SIZE)
    ...
    training_stack = [(50, StandardTrainingModule(criterion=criterion, attack=FGSMAttack(), epsilon=training_parameters.attack_epsilon))]
    ...
    #validation_module = StandardTestModule(attack=pgd_attack, epsilon=ATTACK_EPSILON)
    ...
    experiment_tracker = ExperimentTracker("gtsrb_7_1024-fgsm", Path("./generated"), login=True)

    experiment_tracker.initialize_new_experiment("shuffle", training_parameters=training_parameters | training_objects)

    pipeline = Pipeline(experiment_tracker, training_parameters, criterion, optimizer, scheduler)

    #pipeline.train(train_loader, network, training_stack, in_training_validation_loader=in_training_validation_loader, validation_module=validation_module)
