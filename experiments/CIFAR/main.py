...
CONFIG = {'cifar_7_1024': './config/cifar_7_1024.yaml',
            'conv_big':'./config/cifar_conv_big.yaml',
            'resnet_4b': './config/resnet_4b.yaml', 
            'resnet_18': './config/cifar_resnet18.yaml'}
...
def main():
    ...
    experiment_repository_path = Path(f'./CIFAR')
    
    #TODO: change network folder path as necessary
    network_folder = Path("../networks_onnx/cifar-10")
    ...
    experiment_name = f"JAIR_CIFAR_{split}" #TODO: adjust name as prefered"

    experiment_repository.initialize_new_experiment(experiment_name)

    #Define verifier, we have used abCrown and the one2any property generator (untargeted verification)
    verifier = AutoVerifyModule(verifier=AbCrown(),  timeout=timeout, config = None)
