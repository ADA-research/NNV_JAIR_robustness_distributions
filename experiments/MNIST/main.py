...
def get_balanced_sample(train_bool =True):
    ...
    torch_dataset= torchvision.datasets.MNIST(root="./data", train = train_bool, download=True, transform=data_transforms)

    # Create a subset of the original dataset using the balanced indices
    balanced_dataset = Subset(torch_dataset, [0,100])
...
def main():
    ...
    timeout = 3600
    epsilon_list = np.arange(0.00,0.4,0.002)
    
    #TODO: change to correct path
    experiment_repository_path = Path(f'./MNIST')
    
    #TODO: change network folder path as necessary
    network_folder = Path("../networks_onnx/mnist/cnn")
    ...
    experiment_name = f"JAIR_MNIST_{split}" #TODO: adjust name as prefered"
